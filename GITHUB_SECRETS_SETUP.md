# Настройка GitHub Secrets для автоматического деплоя

## 📋 Необходимые секреты

Для работы автоматического деплоя нужно добавить следующие секреты в GitHub:

### 1. Перейдите в настройки репозитория

- Откройте репозиторий: https://github.com/temakulakov/bx-tg-notify
- Settings → Secrets and variables → Actions → New repository secret

### 2. Добавьте следующие секреты:

#### SSH доступ к серверу:

- **`SSH_PRIVATE_KEY`** - Приватный SSH ключ для доступа к серверу
  - Создайте ключ: `ssh-keygen -t ed25519 -f ~/.ssh/github_deploy -N ""`
  - Добавьте публичный ключ на сервер: `cat ~/.ssh/github_deploy.pub >> ~/.ssh/authorized_keys`
  - Скопируйте приватный ключ: `cat ~/.ssh/github_deploy` (весь вывод)

- **`SERVER_HOST`** - IP адрес или домен сервера
  - Значение: IP адрес вашего сервера (например: `135.125.xxx.xxx`)

- **`SERVER_USER`** - Пользователь для SSH
  - Значение: `root`

- **`APP_DIR`** - Директория приложения на сервере
  - Значение: `/root/bx-tg-notify`

#### База данных:

- **`DB_USER`** - Пользователь MySQL
  - Значение: `shebo` (или ваш пользователь)

- **`DB_PASS`** - Пароль MySQL
  - Значение: ваш пароль MySQL

- **`DB_NAME`** - Имя базы данных
  - Значение: `shebo` (или ваше имя БД)

#### Bitrix24:

- **`BX24_OUTGOING_TOKEN`** - Outgoing webhook token
  - Значение: ваш токен из Bitrix24

- **`BX24_INCOMING_TOKEN`** - Incoming webhook token
  - Значение: ваш токен из Bitrix24

- **`BX24_INCOMING_USER`** - Incoming webhook user ID
  - Значение: ID пользователя (например: `777`)

- **`BX24_DOMAIN`** - Домен Bitrix24
  - Значение: `https://your-domain.bitrix24.ru`

#### Telegram:

- **`TELEGRAM_BOT_TOKEN`** - Токен Telegram бота
  - Значение: токен от @BotFather

---

## 🔑 Создание SSH ключа для деплоя

Выполните на вашем локальном компьютере:

```bash
# Создайте SSH ключ
ssh-keygen -t ed25519 -f ~/.ssh/github_deploy -N ""

# Скопируйте публичный ключ на сервер
ssh-copy-id -i ~/.ssh/github_deploy.pub root@YOUR_SERVER_IP

# Или вручную:
cat ~/.ssh/github_deploy.pub | ssh root@YOUR_SERVER_IP "cat >> ~/.ssh/authorized_keys"

# Скопируйте приватный ключ для GitHub Secrets
cat ~/.ssh/github_deploy
```

**Важно:** Скопируйте весь вывод команды `cat ~/.ssh/github_deploy` (включая `-----BEGIN OPENSSH PRIVATE KEY-----` и `-----END OPENSSH PRIVATE KEY-----`)

---

## ✅ Проверка настройки

После добавления всех секретов:

1. Перейдите в Actions в репозитории
2. Запустите workflow вручную (Run workflow)
3. Или сделайте push в main ветку

---

## 🔒 Безопасность

- ✅ Секреты хранятся в GitHub Secrets (зашифрованы)
- ✅ SSH ключ только для деплоя (можно ограничить права)
- ✅ Приватный ключ никогда не попадает в код
- ✅ `.env` файл создается на сервере и не коммитится

---

## 📝 Пример ограничения SSH ключа

Для дополнительной безопасности можно ограничить SSH ключ только деплоем:

На сервере в `~/.ssh/authorized_keys`:

```
command="cd /root/bx-tg-notify && git pull",no-port-forwarding,no-X11-forwarding,no-agent-forwarding ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAI... github_deploy
```

---

## 🐛 Устранение неполадок

### Ошибка подключения по SSH:

- Проверьте что публичный ключ добавлен в `~/.ssh/authorized_keys`
- Проверьте права: `chmod 600 ~/.ssh/authorized_keys`
- Проверьте что SSH доступен: `ssh root@YOUR_SERVER_IP`

### Ошибка деплоя:

- Проверьте логи в GitHub Actions
- Проверьте логи на сервере: `docker compose logs`
- Проверьте что Docker запущен: `systemctl status docker`

### Ошибка healthcheck:

- Убедитесь что приложение имеет endpoint `/health`
- Проверьте логи: `docker compose logs app`

